...
While in the HW4 I stored optional weather information with the necessary title, (optional) description and image name, I now have laso weatherIcon (ID), latitude and longitude:
...
I ask the permission in the following (in MainActivit before setting the content):
```kotlin
...
I found a good [source]("https://medium.com/@munbonecci/how-to-get-your-location-in-jetpack-compose-f085031df4c1") for this (mentioned also in the code docs). It gave me everything needed, but I had to make some edits to make it sufficient for my purposes. The weather API uses last known user location or default location, if location permissions aren't granted ot the found location is null. The default location is set to be Oulu, but it can be changed from settings, where the current location data is used and it is stored to the shared preferences:
...
Everything from setting are saved to the shared preferences, so the values stay the same even after app is closed. The preferences use hard drive, so I also have each of these settings as variable (in RAM) to keep the app flow smooth.


#### Name

The location sensor gives the latitude and the longitude, but the user won't do anything with that information alone, so I implemented also a Geocode.GeocodeListener for getting the location names (city and country). I first created a Geocode object and used it to get the location names:
...
```kotlin
// The used method is from the same source from above.
...
Basically if location permission is granted, get the location. If not, ask for the permission, if they are given then get the location and if not show the error message to the user (why would the user try to add location, if he don't want to give the location permissions).  
...
I used the default camera application of the device (not CameraX, which is much harder to setup). My application is for storing photos to the diary and not a camera application, so while this approach might not get full points, it was the best in this situation. The camera app be opened the following way:
...
// On click event for opening the camera app and capturing the image.
